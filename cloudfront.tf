resource "newrelic_nrql_alert_condition" "cloudfront_4xx" {
  ...
  nrql {
    query             = "FROM Metric SELECT average(`aws.cloudfront.4xxErrorRate`) WHERE `aws.accountId` IN (${var.aws_account_id}) FACET `aws.cloudfront.DistributionId`, tags.Name"
  }
  critical {
    operator              = "above"
    threshold             = 5
...
resource "newrelic_nrql_alert_condition" "cloudfront_5xx" {
  ...
  nrql {
    query             = "FROM Metric SELECT average(`aws.cloudfront.5xxErrorRate`) WHERE `aws.accountId` IN (${var.aws_account_id}) FACET `aws.cloudfront.DistributionId`, tags.Name"
  }
  critical {
    operator              = "above"
    threshold             = 5
...
resource "newrelic_nrql_alert_condition" "cloudfront_origin_latency" {
  ...
  nrql {
    query             = "FROM Metric SELECT average(`aws.cloudfront.OriginLatency`) WHERE `aws.accountId` IN (${var.aws_account_id}) FACET `aws.cloudfront.DistributionId`, tags.Name"
  }
  critical {
    operator              = "above"
    threshold             = 200
