...
resource "newrelic_nrql_alert_condition" "timesync" {
  ...
  nrql {
    query             = "SELECT abs(latest(timestamp-flex.time.endMs)) AS timeshift FROM flexStatusSample FACET aws.ec2.InstanceId WHERE aws.accountId IN (${var.aws_account_id}) AND tags.${var.ec2_timesync_alerts[count.index].ec2_tag_key} = '${var.ec2_timesync_alerts[count.index].ec2_tag_value}' SINCE 5 minutes ago"
    evaluation_offset = 3
  }
  critical {
    operator  = "above"
...
resource "newrelic_nrql_alert_condition" "memory" {
  ...
  nrql {
    query             = "SELECT average(memoryUsedPercent) FROM SystemSample WHERE aws.accountId IN (${var.aws_account_id}) AND tags.${var.ec2_timesync_alerts[count.index].ec2_tag_key} = '${var.ec2_timesync_alerts[count.index].ec2_tag_value}' FACET aws.ec2.instanceId SINCE 5 minutes ago"
    evaluation_offset = 3
  }
  critical {
    operator  = "above"
...
resource "newrelic_nrql_alert_condition" "network" {
  ...
  nrql {
    ...
    query             = "SELECT (average(aws.ec2.NetworkIn)+average(aws.ec2.NetworkOut)) * 8e-6 / (${var.ec2_network_alerts[count.index].metrics_interval_minutes} * 60) / ${var.ec2_network_alerts[count.index].max_limit_bandwidth_mbps} * 100 FROM Metric WHERE aws.accountId IN (${var.aws_account_id}) FACET aws.ec2.instanceId SINCE 5 minutes ago"
    evaluation_offset = 3
  }
  critical {
    operator  = "above"
