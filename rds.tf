...
resource "newrelic_nrql_alert_condition" "rds_aurora_replica_lag" {
  ...
  nrql {
    query             = "SELECT average(aws.rds.AuroraReplicaLag) FROM Metric WHERE collector.name = 'cloudwatch-metric-streams' AND aws.accountId IN (${data.aws_caller_identity.self.account_id}) FACET aws.rds.DBInstanceIdentifier"
  }
  critical {
    operator           = "above"
    threshold          = 1000 // 1sec
...
resource "newrelic_nrql_alert_condition" "rds_cpu_utilization" {
  ...
  nrql {
    query             = "SELECT average(aws.rds.CPUUtilization) FROM Metric WHERE collector.name = 'cloudwatch-metric-streams' AND aws.accountId IN (${data.aws_caller_identity.self.account_id}) FACET aws.rds.DBInstanceIdentifier"
  }
  critical {
    operator              = "above"
    threshold             = 90
...
resource "newrelic_nrql_alert_condition" "rds_freeable_memory" {
  ...
  nrql {
    query             = "SELECT average(aws.rds.FreeableMemory) FROM Metric WHERE collector.name = 'cloudwatch-metric-streams' AND aws.accountId IN (${data.aws_caller_identity.self.account_id}) FACET aws.rds.DBInstanceIdentifier"
  }
  critical {
    operator              = "below"
    threshold             = 500000000
...
resource "newrelic_nrql_alert_condition" "rds_free_local_storage" {
  ...
  nrql {
    query             = "SELECT average(aws.rds.FreeLocalStorage) FROM Metric WHERE collector.name = 'cloudwatch-metric-streams' AND aws.accountId IN (${data.aws_caller_identity.self.account_id}) FACET aws.rds.DBInstanceIdentifier"
  }
  critical {
    operator              = "below"
...
resource "newrelic_nrql_alert_condition" "rds_database_connections" {
  ...
  nrql {
    query             = "SELECT average(aws.rds.DatabaseConnections) FROM Metric WHERE collector.name = 'cloudwatch-metric-streams' AND aws.accountId IN (${data.aws_caller_identity.self.account_id}) FACET aws.rds.DBInstanceIdentifier"
  }
  critical {
    operator              = "above"
    threshold             = 100
...
resource "newrelic_nrql_alert_condition" "rds_blocked_transactions" {
  ...
  nrql {
    query             = "SELECT average(aws.rds.BlockedTransactions) FROM Metric WHERE collector.name = 'cloudwatch-metric-streams' AND aws.accountId IN (${data.aws_caller_identity.self.account_id}) FACET aws.rds.DBInstanceIdentifier"
  }
  critical {
    operator              = "above"
    threshold             = 10
...
resource "newrelic_nrql_alert_condition" "rds_deadlocks" {
  ...
  nrql {
    query             = "SELECT average(aws.rds.Deadlocks) FROM Metric WHERE collector.name = 'cloudwatch-metric-streams' AND aws.accountId IN (${data.aws_caller_identity.self.account_id}) FACET aws.rds.DBInstanceIdentifier"
  }
  critical {
    operator              = "above"
    threshold             = 10
